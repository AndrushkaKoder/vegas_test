	...
	@foreach($items as $item)
		<li class="dd-item" data-id="{{ $item->id }}">
			...
			-40px">
				...
				<a href="{{ route('admin.navigation.edit', $item->id) }}" class="nestable_action">
					<i class="nav-icon fas fa-pen"></i>
				</a>
				<a class="admin_btn_delete nestable_action" href="{{ route('admin.navigation.destroy', $item->id) }}">
					<i class="nav-icon fas fa-trash"></i>
				</a>
			</div>

			@if($item->childrenSorted->count())
				@include('admin.navigations.index._elements', ['items' => $item->childrenSorted])
			@endif
		</li>
	@endforeach
</ol>
